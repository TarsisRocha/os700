Segue o ai_nlq_ai.py completo (copiar e colar):

# ai_nlq_ai.py
"""
NLQ com IA: interpreta perguntas livres e mapeia para intents SEGURAS.
Não executa SQL. Trabalha em cima de um DataFrame construído a partir de list_chamados().

Requisitos:
- openai>=1.40.0 no requirements.txt
- Definir a chave em:
    • variável de ambiente OPENAI_API_KEY
    • ou em Streamlit Cloud Secrets:
        [openai]
        api_key = "sk-..."

Como usar (no OS800.py):
    from ai_nlq_ai import answer_question
    ...
    result = answer_question(list_chamados(), "qual chamado em aberto mais antigo?")
    st.markdown(result["markdown"])
    if isinstance(result.get("table"), pd.DataFrame):
        st.dataframe(result["table"])

Intents suportadas (todas resolvidas com pandas, sem SQL):
- list_open          : listar chamados em aberto (com filtros opcionais)
- oldest_open        : chamado em aberto mais antigo
- count_open         : contagem de abertos
- open_by_ubs        : abertos por UBS (tabela)
- open_over_hours    : abertos acima de X horas úteis (default 48)
- top_defects        : top tipos de defeito
- avg_resolution     : tempo médio de resolução (horas úteis) de fechados
- open_in_ubs        : abertos em uma UBS específica (string aproximada)
- search_text        : buscar substring em "problema"
- free_summary       : resumo narrativo (IA) do recorte atual (máx 300 linhas enviadas)
"""
